  ...
  const submitHandler = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        `http://3.111.36.104/user/post-review/`,
        {
    ...
    const fetchData = async () => {
      ...
      try {
        const result = await axios.post(
          `http://3.111.36.104/user/particular-product-details/`,
          {
            product_id: id,
          }
        );
    ...
    const reviewData = async () => {
      ...
      try {
        const resultReview = await axios.get(
          `http://3.111.36.104/user/particular-product-reviews/${id}/`
        );
              ...
              >
                {product[0].product_images.map((item) => {
                  return (
                    <SwiperSlide>
                      <img
                        src={"http://3.111.36.104" + item.image}
                        className="small-img"
